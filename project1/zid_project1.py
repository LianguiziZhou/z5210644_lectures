...
def get_tics(pth):
    ...
    with open(pth) as f:
        for line in f:
            (value, key) = line.lower().replace('"', '').strip().split('=')
            # No empty tickers or exchanges added
            if key != "" and value != "":
                tics_dict[key] = value
    return tics_dict
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    tic_line_lst = []
    dat_filename = tic + '_prc.dat'
    dat_pth = os.path.join(DATDIR, dat_filename)
    with open(dat_pth) as f:
        for line in f:
            # Remove newline characters "\n"
            line = line.strip()
            tic_line_lst.append(line)
    return tic_line_lst
...
def line_to_dict(line):
    ...
    start = 0
    for key in COLUMNS:
        end = start + COLWIDTHS[key]
        dat_dict[key] = line[start:end]
        start = end
    ...
    if tickers_lst is not None:
        if len(tickers_lst) == 0:
            ...
            for tic in tickers_lst:
                if tic not in tic_exchange_dic:
                    raise Exception(f"{tic} is not a key of tic_exchange_dic.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    # <COMPLETE THIS PART>
    data_dict = {}
    # Check the invalidity of tickers_lst & col_lst
    if verify_tickers(tic_exchange_dic, tickers_lst) is None and verify_cols(col_lst) is None:
        # Check if tickers_lst & col_lst is None
        ...
        for tic in tickers_lst:
            value_dict = {'exchange': tic_exchange_dic[tic], 'data': []}
            for line in range(len(read_dat(tic))):
                col_dict = {}
                for col in col_lst:
                    col_dict[col] = line_to_dict(read_dat(tic)[line])[col]
                value_dict['data'].append(col_dict)
                line += 1
            data_dict[tic] = value_dict
