...
def read_prc_csv(tic):
    ...
    """
    # <COMPLETE THIS PART>
    # Get the tic_csv file path
    tic_csv = tic.upper() + '_prc.csv'
    tic_csv_pth = os.path.join(cfg.DATADIR, tic_csv)

    tic_tmp_df = pd.read_csv(tic_csv_pth, index_col='Date')
    # Formatted column names acc to `standardise_colnames` in cfg
    tic_df = cfg.standardise_colnames(tic_tmp_df)
    return tic_df
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for tic in tickers:
        tic_df = read_prc_csv(tic)
        tic_ser = tic_df[prc_col]
        # Add the tic as key, prc_col as value to tics_ser_dic
        tics_ser_dic.update({tic: tic_ser})

    prc_df = pd.DataFrame(tics_ser_dic)
    return prc_df
...
def mk_ret_df(prc_df):
    ...
    ff_tmp_df = pd.read_csv(cfg.FF_CSV, index_col='Date')
    ff_df = cfg.standardise_colnames(ff_tmp_df)

    mkt_ser = ff_df['mkt']
    mkt_df = pd.DataFrame({'mkt': mkt_ser})
    # Add the mkt to prc_df and keep the original dates in prc_df
    ret_df = pd.concat([prc_df, mkt_df], axis=1, join='inner')
    return ret_df
